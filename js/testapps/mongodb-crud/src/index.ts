...
import { mongodb, mongodbIndexerRef } from 'genkitx-mongodb';
...
async function main() {
  ...
  // Please insert a new document with name "John Doe", email "john@example.com", age 25 using the available tool.
  // With the above insertedId, find the document by id and return the document.
  // Now delete the document by id using the available tool.
  try {
    const { text } = await ai.generate({
      model: googleAI.model('gemini-2.0-flash'),
      prompt: `
      Using the available tools,
      Please insert a new document with {"name": "John Doe", "email": "john@example.com", "age": 25}.
      With the above insertedId, find the document by id and return the document.
      Now update the document with the inserted id to update: {$set: {age: 12} using the available tool.
      Now delete the document by id
      `,
      tools: [
